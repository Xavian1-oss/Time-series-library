...
class Exp_CRC(Exp_Long_Term_Forecast):
    ...
    def _build_model(self):
        # 覆盖父类默认行为：CRC 不用 self.model
        return None

    def _get_predictions(self, model, data_loader):
        """
        用基线模型在 loader 上做推理，返回：
        preds (B,H,N), trues (B,H,N), inputs W (B,P,N)
        """
        model.eval()
        ...
        with torch.no_grad():
            for i, (batch_x, batch_y, batch_x_mark, batch_y_mark) in enumerate(data_loader):
                batch_x = batch_x.float().to(self.device)  # (B,P,N)
                batch_y = batch_y.float().to(self.device)  # (B, L+H, N)

                # decoder input（与原基座一致）
                dec_inp = torch.zeros_like(batch_y[:, -self.args.pred_len:, :]).float()
                dec_inp = torch.cat([batch_y[:, :self.args.label_len, :], dec_inp], dim=1).to(self.device)

                # 大部分仓库里的 encoder-decoder 基座签名: model(x, x_mark, dec_inp, y_mark)
                outputs = model(batch_x, None, dec_inp, None)

                f_dim = -1 if self.args.features == 'MS' else 0
                outputs = outputs[:, -self.args.pred_len:, f_dim:]   # (B,H,N)
    ...
    def train(self, setting):
        ...
        yhat_tr_bl, y_tr_win, W_tr = self._get_predictions(self.baseline_model, train_loader)  # (B,H,N),(B,H,N),(B,P,N)
        ...
        # 2) Stage A：MTL 预训练（不确定性加权，无需 alpha_*）
        ...
        mtl_pretrain_trainer(
            ...
            # 其他超参用默认：mask_ratio=0.15, lr=1e-3, wd=1e-4, ...
        )

        # 3) 训练 Hybrid 纠偏
        print("\n--- Stage B: Hybrid Corrector (multi-scale priors) ---")
        # 先用训练集一次性生成多尺度先验来获取 mu/sd（只用训练集，防泄露）
        ...
        self.hybrid_corrector = HybridTrainer(
            ...
            H=self.args.pred_len, N=self.args.enc_in, P=self.args.seq_len,
    ...
    def test(self, setting):
        ...
        yhat_te_bl, y_te, W_te = self._get_predictions(self.baseline_model, test_loader)

        print("使用 Hybrid Corrector 对基线预测进行校正...")
        yfinal_te = self.hybrid_corrector.predict(W_te, yhat_te_bl)
